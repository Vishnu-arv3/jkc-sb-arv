...
CREATE TABLE public.videos (
  ...
  title TEXT NOT NULL DEFAULT 'My Home Journey',
...
CREATE POLICY "Users can upload selfies" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'selfies' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "Selfies are publicly accessible" ON storage.objects FOR SELECT USING (bucket_id = 'selfies');
CREATE POLICY "Users can delete their own selfies" ON storage.objects FOR DELETE USING (bucket_id = 'selfies' AND auth.uid()::text = (storage.foldername(name))[1]);